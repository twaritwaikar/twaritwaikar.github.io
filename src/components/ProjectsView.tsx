...
import { ExternalLink, Lock, Wrench } from 'lucide-react';
...
export const ProjectsView: React.FC<ProjectsViewProps> = ({
  ...
  return (
    <div id="projects_view_container" className="h-full min-h-0 flex flex-col gap-3 max-w-[1280px] mx-auto overflow-hidden">
      ...
      <section
        id="projects_grid"
        className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-3 min-h-0 overflow-hidden"
      >
        {projects.map((project) => {
          return (
            <div
              ...
              className={`border border-[#2a2a2a] flex flex-col justify-between min-h-0 ${
                isDarkMode ? 'bg-[#141414]' : 'bg-[#EAEAEA]'
              }`}
            >
              <div className="min-h-0 flex flex-col">
                <div className="px-3 py-2 bg-[#111111] border-b border-[#2a2a2a] flex items-center justify-between font-mono text-xs text-neutral-300">
                      ...
                      {(['PROJECTS', 'ANALYTICS', 'CONTRIBUTIONS', 'SETTINGS'] as const).map((tab) => (
                        <button
                          ...
                          className={`px-1.5 py-0.5 uppercase cursor-pointer ${
                            activeOpsTab === tab
                              ? 'text-white border-b border-[#5CE883] font-bold'
                ...
                <div className="p-3 flex-1 min-h-0">
                  <p className="font-sans text-xs text-neutral-600 dark:text-neutral-400 leading-relaxed line-clamp-3">
                  ...
                  <div className="flex flex-wrap gap-1.5 mt-3">
                    {project.tags.map((tag) => (
                      <span
                        key={tag}
                        className="px-2 py-0.5 font-mono text-[10px] uppercase tracking-wider border border-[#333333] bg-[#1a1a1a] text-neutral-300"
                      >
                        {tag}
                      </span>
                    ))}
              ...
              <div className="p-2 bg-[#111111] border-t border-[#2a2a2a] grid grid-cols-3 gap-1.5 font-mono text-[10px]">
                <button
                  ...
                  className="py-1.5 px-2 border border-[#333333] hover:border-[#5CE883] hover:text-[#5CE883] bg-[#161616] text-neutral-300 flex items-center justify-center gap-1 transition-colors cursor-pointer"
                >
                  <span>&lt; &gt;</span>
                ...
                {project.status === 'LIVE' && (
                  <button
                    ...
                    className="py-1.5 px-2 border border-[#5CE883] text-[#5CE883] hover:bg-[#5CE883] hover:text-black bg-[#161616] flex items-center justify-center gap-1 transition-colors cursor-pointer"
                  >
                ...
                {project.status === 'OFFLINE' && (
                  <button
                    ...
                    className="py-1.5 px-2 border border-neutral-700 text-neutral-400 bg-[#161616] flex items-center justify-center gap-1"
                  >
                ...
                {project.status === 'BUILD_FAIL' && (
                  <button
                    ...
                    className="py-1.5 px-2 border border-amber-900/60 text-amber-500/80 bg-[#161616] flex items-center justify-center gap-1"
                  >
